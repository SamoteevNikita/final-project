...
// Copy html files 
function copyHtml() { 
  return src(paths.html.app).pipe(dest(paths.html.dest)); 
} 
 ...
 */ 
function cacheBust() { 
  return src(paths.cachebust.app) 
...
function watcher() { 
  watch(paths.html.app, series(copyHtml, cacheBust)); 
  watch(paths.images.app, optimizeImages); 
  watch(paths.styles.app, parallel(compileStyles, cacheBust)); 
  watch(paths.scripts.app, parallel(minifyScripts, cacheBust)); 
} 
